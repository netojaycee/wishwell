...
- Try at least two different occasions, pick one **celebratory** one (birthday, wedding,
  ...
  see that.
- On the "Tell us about them" step, add a short bio and **1 to 4 photos** of the person (or
  anything that does them justice). Try making a different photo the main one, removing one,
  and pressing Continue with the name empty (you should see a clear message, not a browser
  popup). Check the photos and bio then appear on the board and on the "add your message" page
...
- Once signed in, the top-right shows your initials (or Google photo) with a small arrow:
  open it and check it shows your email, "My boards", "New board" and "Sign out"
